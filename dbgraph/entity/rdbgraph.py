...
class RDatabaseGraph(DatabaseGraph):
    ...
    def get_refs_with_columns(self, table_id: str) -> RDatabaseGraph:
        ...
        target_table = self._nodes_data[self._asset_id_to_node_idx[table_id]]
        ...
        links: list[Link] = [
            self._graph.get_edge_data(
                self._asset_id_to_node_idx[target_table.asset_id], self._asset_id_to_node_idx[column.asset_id]
            )
        ...
        for table in refs_tables:
            try:
                fk_link = self._graph.get_edge_data(
                    self._asset_id_to_node_idx[table_id], self._asset_id_to_node_idx[table.asset_id]
                )
            except NoEdgeBetweenNodes as e:
                print(self._asset_id_to_node_idx[table_id], self._asset_id_to_node_idx[table.asset_id])
                raise e
            ...
            for column in columns:
                ...
                link = self._graph.get_edge_data(
                    self._asset_id_to_node_idx[table.asset_id], self._asset_id_to_node_idx[column.asset_id]
                )
                links.append(link)
